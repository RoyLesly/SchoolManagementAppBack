...
from .serializers import (
    UserCreateSerializer, UserUpdateSerializer, CustomUser, UserActivities, 
    UserActivitiesSerializer, GetUserSerializer, GroupSerializer, PermissionSerializer,
)
...
class PageUserActivitiesView(ModelViewSet):
...
# KPI
# class KpiUserControlView(ViewSet):
#     http_method_names = ["get", ]
...
class KpiYearlyModelCountListView(ViewSet):
    ...
    def list(self, request):
        ...
        if (model == "CustomUser"):
            ...
            results = model.objects.values("created_at__year").annotate(**metrics)
            l = (list(results))
            ...
            results = model.objects.values("specialty__academic_year").annotate(**metrics)
            l = (list(results))
...
class KpiUserProfileSpecialtyCountTwoView(ViewSet):
    ...
    def list(self, request):
        ...
        academic_years = list(set([f.specialty.academic_year for f in user_profiles_with_specialties]))                                         # USE LIST COMPREHENSION & SET
        data = []
        for s in specialty_names:
            item = []
            for y in academic_years:
